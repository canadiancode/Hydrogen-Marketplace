...
function generateStructuredData(baseUrl) {
  ...
  return {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${safeBaseUrl}/general-privacy-policy`,
    },
    inLanguage: 'en-US',
  };
}
...
export const meta = ({request}) => {
  ...
  const canonicalUrl = `${baseUrl}/general-privacy-policy`;
...
export async function loader({request}) {
  ...
  try {
    ...
    if (duration > 100) {
      console.warn(`Slow loader: general-privacy-policy took ${duration}ms`);
    }
          ...
          <div className="mb-8">
            <Breadcrumbs 
              items={[
                ...
                {name: 'Privacy Policy', href: '/general-privacy-policy', current: true},
              ]}
            />
          </div>
...
/** @typedef {import('./+types/general-privacy-policy').Route} Route */
